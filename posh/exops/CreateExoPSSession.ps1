...
function Connect-EXOPSSession {
    <#
        .SYNOPSIS
            ...
            - EnableEXOTelemetry To collect telemetry on Exchange cmdlets. Default valus is False.
    ...
    param(
        ...
        [switch] $EnableEXOTelemetry = $false, 
    ...
    try
    {
        ...
        $PSSession = New-ExoPSSession -UserPrincipalName $UserPrincipalName -ConnectionUri $ConnectionUri -AzureADAuthorizationEndpointUri $AzureADAuthorizationEndpointUri -PSSessionOption $PSSessionOption -Credential $Credential -BypassMailboxAnchoring:$BypassMailboxAnchoring
        ...
        if ($PSSession -ne $null)
        {
            ...
            if ($EnableEXOTelemetry -eq $true)
            {
                Add-EXOClientTelemetryWrapper -Organization (Get-OrgNameFromUPN -UPN $UserPrincipalName) -PSSessionModuleName $PSSessionModuleInfo.Name -TelemetryFilePath $TelemetryFilePath -DoLogErrorMessage:$DoLogErrorMessage
            }
        }
    }
    catch
...
function Connect-IPPSSession
{
    ...
    Connect-EXOPSSession -ConnectionUri $newUri -AzureADAuthorizationEndpointUri $AzureADAuthorizationEndpointUri -UserPrincipalName $UserPrincipalName -PSSessionOption $PSSessionOption -Credential $Credential -BypassMailboxAnchoring:$BypassMailboxAnchoring
}